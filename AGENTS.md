...
### The correct way to grab base64 strins to variables
...
# ===== Screenshot with Puppeteer (heredoc avoids bash expansion) =====
node <<'NODE_SCRIPT'
...
(async () => {
  ...
  const full = await page.screenshot({ fullPage: true, encoding: 'base64' });
  fs.writeFileSync(path.join(OUTPUT_DIR, 'screenshot.base64'), full);
...
# ===== Insert full screenshot (Perl reads from file, no huge arguments) =====
perl -i -pe '
  BEGIN {
...
for b64_file in "${OUTPUT_DIR}"/section-*.base64; do
  if [ -f "$b64_file" ]; then
    ...
    echo "<div><img src=\"data:image/png;base64,${b64_data}\" alt=\"${id}\" /><p>${id}</p></div>" >> "$GALLERY_TMP"
  fi
done
...
## Design System & Implementation Guide for Modern Single‑Page Websites

This document is a condensed representation of a premium front‑end design system, derived from a comprehensive codebase and skill library. It is optimised for building a single, beautiful, high‑quality HTML page from scratch. All framework‑specific (Next.js, React) details have been removed; only universal HTML/CSS/JS principles remain.

---

### Purpose

To provide AI systems and developers with a clear, actionable design language and set of implementation rules that guarantee a visually stunning, conversion‑aware, and anti‑generic single‑page website. The output should score 10/10 on quality, avoiding common AI tells and delivering a premium, art‑directed experience.

---

### Core Design Philosophy

* Intentional, minimal, and premium. Every element earns its place.  
* Anti‑default. Actively reject AI clichés: purple gradients, centered heroes, three equal cards, Inter font, generic glassmorphism, etc.  
* Art‑directed. Use real imagery, texture, and asymmetry.  
* Conversion‑focused. Even artistic sites must have a clear funnel: hook → proof → action.  
* Performance & accessibility. Motion respects reduced‑motion; contrast meets WCAG AA; all interactive elements have feedback.

---

### Color Palette (CSS Variables)

Use these tokens in your `:root` and `.dark` selectors. Never use pure black (`#000000`) or pure white (`#ffffff`) – use off‑black and off‑white.
...
`:root {`  
  ...
  `--outline: #EAEAEA;           /* borders, dividers */`  
  `--on-bg-high: #18181B;        /* primary text (zinc-950) */`  
  `--on-bg-medium: #71717A;      /* secondary text */`  
  `--on-bg-low: #94A3B8;         /* tertiary / placeholder */`  
  `--primary: #336DFF;           /* accent (electric blue) */`  
  `--on-primary: #FFFFFF;`  
  `--primary-card: #E2ECFF;`  
  `--on-primary-card: #1A2B5C;`  
...
`.dark {`  
  ...
  `--on-primary: #FFFFFF;`  
  `--primary-card: #1A2B5C;`  
  `--on-primary-card: #E2ECFF;`  
  `--state-hover: color-mix(in srgb, #FFFFFF, transparent 95%);`  
  `--state-focus: color-mix(in srgb, #FFFFFF, transparent 92%);`

`}`

Accent rule: Use one accent color (here `#336DFF`) for CTAs, focus rings, and active states. Saturation \< 80%. No purple/neon gradients unless explicitly requested.
...
* Primary sans‑serif: `Geist`, `Satoshi`, `Cabinet Grotesk`, or `Outfit` – never `Inter`.  
* Optional serif (only for editorial/luxury): Use distinctive modern serifs like `Fraunces`, `Instrument Serif`, or `PP Editorial New` – never generic `Times New Roman` or `Georgia`.  
* Monospace (for code/data): `Geist Mono`, `JetBrains Mono`.

#### Scale

Use `clamp()` for fluid sizing. Example:

* Display: `clamp(2.25rem, 5vw, 3.75rem)` – weight 700, line‑height 1.1, tracking `-0.025em`.  
* Body: `1rem` – weight 400, line‑height 1.65, max‑width 65ch, color `var(--on-bg-medium)`.  
* Labels: `0.8125rem` – weight 500, tracking `0.05em`.

Heading rule: Never exceed 2 lines on desktop. Use weight and colour for hierarchy, not just size.

---

### Components (Pure CSS/HTML)

#### Buttons

* Primary: `background: var(--primary); color: var(--on-primary);` – flat, no outer glow.  
* Secondary: `border: 1px solid var(--outline); background: transparent;` – ghost/outline.  
* Active feedback: `transform: scale(0.98);` on `:active`.  
* Size: `padding: 0.5rem 1.5rem; border-radius: 0.5rem;` (or full pill for premium).  
* Always ensure contrast (AA 4.5:1).

#### Cards

* Use only when elevation communicates hierarchy. Otherwise, use `border-top` or spacing.  
* Style: `background: var(--card); border: 1px solid var(--outline); border-radius: 2.5rem; padding: 2rem; box-shadow: 0 20px 40px -15px rgba(0,0,0,0.05);`  
* High‑density layouts: avoid cards; use dividers and spacing.

#### Inputs & Forms

* Label above input, error below.  
* Input: `border: 1px solid var(--outline); background: var(--bg); padding: 0.5rem; border-radius: 0.5rem;`  
* Focus ring: `outline: 2px solid var(--primary);`  
* Placeholder: `color: var(--on-bg-low);`

#### Badges / Tags

* Rounded‑full (`border-radius: 9999px`), small text (`text-xs`), uppercase with wide tracking.  
* Use only when semantically necessary.

#### Navigation

* Sticky header: `height: 70px; background: var(--card-glass); backdrop-filter: blur(10px); border-bottom: 1px solid var(--outline);`  
* Desktop: horizontal links, single line.  
* Mobile: hamburger menu (slide‑in or overlay) with stagger reveal.

#### Hero Section

* Layout: Asymmetric split (text left / visual right), centered over full‑bleed image, or bottom‑left over image – never centered text over dark mesh as default.  
* Content: Eyebrow (optional) \+ headline (≤ 2 lines) \+ subtext (≤ 20 words, ≤ 4 lines) \+ CTAs (max 2).  
* Image: Use a real photo (generated or `picsum.photos/seed/{keyword}/1600/900`). Apply subtle overlay for readability.  
* No filler text ("Scroll to explore") or fake badges.

#### Social Proof / TrustBar

* Real company logos (Simple Icons or generated monograms). Logo‑only – no category labels.  
* Place below hero, not inside it.

#### Features / Bento Grid

* Use CSS Grid with asymmetric cell sizes (`2fr 1fr`, `1fr 1fr 2fr`, etc.).  
* No 3 equal cards in a row – prefer zig‑zag, 2‑column with varied heights.  
* At least 2‑3 cells with images or gradients – avoid all‑white text boxes.  
* Mobile: collapse to single column with `gap: 1.5rem`.

#### Quotes / Testimonials

* Quote body: ≤ 3 lines.  
* Attribution: name \+ role \+ company.  
* Use real typographic quotes (“ ”) or none.

#### Footer

* Simplified: logotype \+ social icons \+ copyright.  
* Avoid 4‑column link farms.
...
* Grid: Use CSS Grid (`grid-template-columns: 1fr 1fr;`) – never `calc()` percentage hacks.  
* Full‑height sections: `min-height: 100dvh;` – never `height: 100vh;` (iOS bug).  
* Mobile collapse (\< 768px): All multi‑column → single column; `padding: 1rem;`; no horizontal scroll.  
* Touch targets: Minimum `44px` for interactive elements.  
* Spacing: Vertical gaps `clamp(3rem, 8vw, 6rem)` – generous.

---

### Motion & Interaction (Pure CSS \+ Vanilla JS)

#### Defaults

* Easing: `cubic-bezier(0.16, 1, 0.3, 1)` or spring‑like.  
* Entry animations: Elements fade in (`opacity: 0 → 1`) and translate up (`translateY(20px) → 0`) when they enter viewport. Use `IntersectionObserver` – never `window.addEventListener('scroll')`.  
* Hover: Subtle scale (`1.02`) or shadow shift.  
* Active: Press feedback (`scale(0.98)`).  
* Reduced motion: Wrap animations in `@media (prefers-reduced-motion: no-preference)`.

#### Advanced (Optional)

* Sticky‑stack cards: Pin sections with `position: sticky` and transform previous cards.  
* Horizontal scroll‑hijack: Pin container and translate inner track on scroll.  
* Marquee: Only once per page, using `animation-timeline: view()` or JS fallback.
...
* No decorative pills/labels over images.  
* No version footers (`v1.2.3`).  
* No locale/time strips (unless travel‑focused).  
* No duplicate CTA intent (e.g., "Contact us" \+ "Get in touch" on same page – pick one).
...
* □ Hero has a clear focal point, not a mess.  
* □ Font is not Inter.  
* □ Color palette is not default purple‑blue.  
* □ Layout is not symmetric/centered everywhere.  
* □ Cards are used sparingly and with purpose.  
* □ Images are real (not div‑based fakes).  
* □ No em‑dashes anywhere.  
* □ No “scroll to explore”.  
* □ No three‑column feature row.  
* □ Buttons have proper contrast and press feedback.  
* □ Social proof uses real logos, not text wordmarks.  
* □ Quotes are short and attributed.  
* □ Footer is clean and minimal.

---

### Implementation Notes for a Single HTML Page
...
1. Hero – strong value proposition, primary CTA, art‑directed image.  
2. Trust Bar – logos of customers / partners (logo‑only).  
3. Features / Bento – 4‑5 key benefits in an asymmetric grid.  
4. Product / Service Showcase – image \+ text (varied layout).  
5. Social Proof – testimonials or case studies (≤ 3 quotes).  
6. Pricing / Plans (if applicable) – clean, with a recommended tier highlighted.  
7. FAQ (optional) – accordion with clear questions/answers.  
8. Final CTA – bold, minimalist, with a single action.

Each section must have distinct layout rhythm – do not repeat the same pattern twice in a row.

### Final Pre‑Flight Check (Before Code Output)

* □ Design Read declared (e.g., "Premium SaaS for technical buyers, Linear‑style minimalism").  
* □ Color palette locked (one accent, consistent neutrals).  
* □ Typography chosen (not Inter, serif only if justified).  
* □ Hero fits viewport (headline ≤ 2 lines, subtext ≤ 20 words, CTA visible).  
* □ No duplicate CTA intent.  
* □ Eyebrow count ≤ ceil(sections/3).  
* □ Motion is motivated (not just for show).  
* □ All images are real (or placeholders with explicit notes).  
* □ Reduced motion respected.  
* □ Dark mode tested (if implemented).  
* □ Code is clean, semantic, and accessible.  
* □ Page passes the “AI Tell” checklist above.

---

The following files (skills) are included in full, as they contain the detailed, battle‑tested directives that guarantee a 10/10 outcome. Use them as your primary source for visual and interaction design.
